...
public void doPost(HttpServletRequest request, HttpServletResponse response)
	...
	AdminLoginTO lt=new AdminLoginTO();
...
float a=3,b=4;
	...
	try {
		...
		role = new AutomationDelegate().loginCheck1(lt);
   
		System.out.println("in LoginAction Role is.........." + role);

		 if(role!=null){
			 
			session.setAttribute(UtilConstants._LOGINUSER,role );
			session.setAttribute(UtilConstants._ADMINROLE, role);
			//session.setAttribute(UtilConstants._PASSWORD, pass);
			target = UtilConstants._adminhome;
			System.out.println("sreenu::::::mahi"+role);
			...
			session.setAttribute(UtilConstants._ADMINROLE, role);
			request.setAttribute("adminloginfail","Give validate admin details");
			target = UtilConstants._adminLogin;
		...
		RequestDispatcher rd = request
				.getRequestDispatcher(UtilConstants._adminLogin);
